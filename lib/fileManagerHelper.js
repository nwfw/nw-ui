...
class FileManagerHelper extends AppBaseClass {
    ...
    getCachedFileList (instanceId, listPath){
        ...
        if (data && data.fm && data.fm.currentDir){
            if (!listPath){
        ...
        if (data && data.fm) {
            ...
            for (let i=0; i<fileList.length; i++){
                if (!(!data.fm.config.showHiddenFiles && fileList[i].match(/^\./))){
                    ...
                    let isItemFile = _appWrapper.fileManager.isFileSync(filePath);
                    if (isItemFile) {
                    ...
                    if (addFile){
                        if (!data.fm.settings.listDirectories && !isItemFile){
                            addFile = false;
                        }
                        if (!data.fm.settings.listFiles && isItemFile){
                            addFile = false;
                        }
                    }
    ...
    async loadTree (instanceId){
        let data = this.getInstanceData(instanceId);
        if (data && data.fm && data.fm.settings.showTree){
            data.fm.totalDirs = 1;
    ...
    scrollTree (instanceId){
        let data = this.getInstanceData(instanceId);
        if (data && data.fm && data.fm.settings && data.fm.settings.scrollTree){
    ...
    canSelect (instanceId){
        let data = this.getInstanceData(instanceId);
        let result = false;
        let settings = data.fm.settings;
        if (settings.allowSelecting){
            if (settings.maxSelectedItems > 0) {
                if (data.fm.selectedFiles.length == 1 && settings.maxSelectedItems == 1) {
                    result = true;
                } else if (data.fm.selectedFiles.length < settings.maxSelectedItems){
                    result = true;
                }
            } else {
                result = true;
    ...
    getDirUpItem (instanceId){
        ...
        let itemPath = '..';
        if (data && data.fm && data.fm.currentDir){
            itemPath = path.resolve(path.join(data.fm.currentDir, '..'));
        }
        let item = {
            name: '..',
            newName: '..',
